...
#### Paso 1: Instalar MS-MPI

1. Visite el sitio oficial de Microsoft MPI:
   [Microsoft MPI](https://www.microsoft.com/en-us/download/details.aspx?id=57467)

2. Descargue estos dos archivos:
   - `msmpisetup.exe`
   - `msmpisdk.msi`

3. Instale los archivos en este orden:
   - Primero `msmpisetup.exe`
   - Luego `msmpisdk.msi`

La instalación de MPI está completa cuando ambos archivos se hayan instalado correctamente.
...
mpiexec
```
Si ve un mensaje que comienza con "Microsoft MPI", la instalación fue exitosa.

#### Solución de problemas comunes

**Error: mpicc no se reconoce como comando**
- Agregue la siguiente ruta al PATH del sistema:
  `C:\Program Files\Microsoft MPI\Bin`

**Error: Falta de archivos DLL**
- Reinicie el sistema
- Si persiste, reinstale ambos componentes de MS-MPI
...
mpicc -O3 -o programa programa.c

# Compilar con información de depuración
mpicc -g -o programa programa.c
```

### Ejecución
```bash
# Ejecutar con 4 procesos
mpiexec -n 4 ./programa

# En algunos sistemas también puede usar mpirun
mpirun -np 4 ./programa

# Para Windows
mpiexec -n 4 programa.exe
```

Notas importantes:
- El parámetro `-n` o `-np` especifica el número de procesos
...
- [Peter Pacheco's Book](https://www.cs.usfca.edu/~peter/ppc/)
